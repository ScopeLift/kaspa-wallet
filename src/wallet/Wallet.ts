...
class Wallet {
  ...
  constructor(privKey?: string) {
    // Disable lint errors in the constructor
    if (privKey) {
  ...
  static async import(password: string, encryptedMnemonic: string): Promise<Wallet> {
    const decrypted = String(await passworder.decrypt(password, encryptedMnemonic));
   ...
   * @returns Object-like string of encrypted mnemonic
   */
  async export(password: string): Promise<string> {
    const encryptedMnemonic = await passworder.encrypt(
      password,
      Buffer.from(String(this.mnemonic), 'utf8')
    );
